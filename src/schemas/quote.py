"""Price Quote & Discount Clamping Schemas."""
from typing import List, Dict, Any, Optional
from pydantic import BaseModel, Field


class QuoteRequestItemSchema(BaseModel):
    ...
    requested_discount_pct: float = Field(
        ...
        ge=0.0,
        le=100.0,
        description="Requested discount percentage for this item",
    )


QuoteRequestItemDTO = QuoteRequestItemSchema


class QuoteRequestInputSchema(BaseModel):
    items: List[QuoteRequestItemSchema] = Field(..., min_length=1)
    overall_requested_discount_pct: float = Field(
        default=0.0,
        ge=0.0,
        le=100.0,
        description="Storewide requested discount percentage",
    )
...
QuoteRequestInputDTO = QuoteRequestInputSchema
...
QuoteResponseDTO = QuoteResponseSchema
